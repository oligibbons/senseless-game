...
export default function GameOverPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  useEffect(() => {
    ...
    const fetchGameData = async () => {
      ...
      // 2. Calculate Funny Awards Client-Side (Zero-Cost Compute)
      ...
      // -- Paranoid Android (Most Wrong Guesses)
      ...
      // -- Unjustly Accused (Most innocent votes received)
      ...
      if (masterThief && (masterThief.stats?.successful_steals || 0) > 0) {
        calculatedAwards.push({
          ...
          description: `Successfully stole points ${masterThief.stats?.successful_steals} times after getting caught.`,
          color: "orange"
        });
      }
      ...
      // 3. Shuffle the awards to keep the UI fresh and unpredictable
    ...
    // Listen for the host clicking "Play Again"
  ...
  // Staggered pop-in animations for that punchy UI feel
      ...
      <div className="flex flex-col h-full p-4 relative text-center">
        ...
        <div className="mt-4 mb-2 shrink-0">
          <motion.h1 
            ...
            className="font-display text-6xl text-toxic-green drop-shadow-chunky leading-none text-outline text-white"
          >
            GAME OVER
          </motion.h1>
        </div>
        ...
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.2 }} className="mb-4 shrink-0">
          <SlimeBox color="yellow" className="!min-h-[140px] !p-6">
            ...
            <h2 className="font-display text-5xl text-white text-outline leading-none mb-2">{winner.player_name}</h2>
            ...
            <motion.div variants={itemVariants} key={idx}>
              <SlimeBox color={award.color} className="!min-h-[100px] !p-4 flex flex-col justify-center relative overflow-hidden">
                <div className="relative z-10 text-left">
                  ...
                  <p className="font-sans text-white text-sm font-bold text-outline mt-2 leading-tight">{award.description}</p>
                </div>
              </SlimeBox>
            </motion.div>
          ))}
        ...
        <div className="mt-auto pt-2 space-y-2 shrink-0">
          {isHost ? (
             ...
             >
                <span className="font-display text-4xl text-white text-outline tracking-wider leading-none">
                  {isResetting ? "RESETTING..." : "PLAY AGAIN"}
                </span>
             </SlimeBox>
          ) : (
            <SlimeBox color="pink" className="!min-h-[90px] !p-2 animate-pulse">
          ...
          <button 
            ...
            className="w-full bg-white text-bruise-purple font-display text-2xl py-3 rounded-xl border-4 border-bruise-purple shadow-[4px_4px_0px_0px_#12001A] transition-transform active:scale-95 uppercase tracking-widest"
          >
            QUIT TO MENU
          </button>
        </div>
